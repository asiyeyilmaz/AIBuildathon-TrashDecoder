...
**"Neyi, Nereye Atıyorum?"** belirsizliğini ortadan kaldıran, CLAUDE AI destekli, konum duyarlı atık yönetim asistanı.
...
## 💡 Yapay Zeka Çözümü (Claude API)
Bu proje, karmaşık veri setleri eğitmek yerine **Claude**'Nin multimodal (görsel + metin) yeteneklerini kullanır:

1.  **Nesne Tanıma:** Kullanıcı fotoğrafı çeker; Claude nesnenin materyalini ve durumunu (temiz/kirli/yağlı) analiz eder.
2.  **Konum Bazlı Dinamik Karar:** Uygulama, kullanıcının konumunu (GPS) alarak o bölgedeki belediyenin güncel atık yönetmeliğini Claude'a "bağlam" (context) olarak sunar.
3.  **Akıllı Yönlendirme:** Claude, "Bu bir pizza kutusu ama yağlı olduğu için Kadıköy Belediyesi kurallarına göre kağıt yerine evsel atığa gitmeli" gibi açıklayıcı yanıtlar verir.

## 🚀 Neden Farklı?
* **Eğitim Seti Gerektirmez:** Claude API sayesinde en güncel nesne tanıma teknolojisini kullanır.
...
## 🛠️ Teknik Mimari
* **Model:** Claude Sonnet (Hızlı ve maliyet etkin görsel analiz için).
* **Frontend:** Flutter / React Native (Mobil öncelikli).
* **Backend:** Node.js (Konum verisini kural setleriyle eşleştirmek için).
